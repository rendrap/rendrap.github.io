...
If you have a Gemfile, [use Bundler]about-bundler by typing `bundle exec jekyll serve` instead.
{: .note .info}
...
Note that in looking for templates, you want the HTML output of the template. If the template has PHP tags or other dynamic scripts, these dynamic elements will need to be converted to HTML or to [Liquid](https://shopify.github.io/liquid/). Liquid is [Jekyll templating system] to retrieve dynamic content.
...
Jekyll provides some [filters](}#filters) to prepend a site URL before path. For example, you could preface your stylesheet like this:
...
You can even set [default front matter tags]#front-matter-defaults) for pages, posts, or [collections] in your `_config.yml` file so that you don't have to specify the layout in the front matter variables. Anywayd, setting defaults is beyond the scope of this tutorial, let's get back to work.
...
You can also package your layouts, includes and assets into a Ruby `gem` and [make it a Jekyll theme]#creating-a-theme).
